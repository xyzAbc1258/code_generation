...
import generation.exprs.ExprStringBuilder
import generation.exprs.{Expr => MExpr}
import shapeless._
...
object MGenerator {
  ...
  trait MacroSimplImpl extends GeneratorTypes {
    ...
    def generatePair(implicit ctx: ExecCtx): Set[Candidate] = {
      if (ctx.result proper_<:< Types.pairType) Cache.cached(Stage.PairGen) { //Is result a pair ?
    ...
    def generateSimpleComposite(implicit ctx: ExecCtx): Set[Candidate] = {
      if (ctx.result proper_<:< Types.hlistType) generateCompositeHList
      else if (ctx.result proper_<:< Types.cconsType) generateCompositeCoproduct
      else Set()
    }
