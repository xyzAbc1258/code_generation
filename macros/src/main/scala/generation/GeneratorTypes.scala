...
import generation.exprs.ExprBuilderGeneric
import generation.exprs.ExprTree
import generation.exprs.ExprType
import generation.utils.AllEqualWrapper
...
trait GeneratorTypes extends FunctionProviders {
  ...
  object ExprCreate {
    ...
    val ns = q"_root_.generation.exprs"
  ...
  object Candidate {
    ...
    private val exprTpe: Type = weakTypeOf[generation.exprs.Expr[_, _, _]]
  ...
  def typeToFuncProvider(t: Type, idx: Int, subIndex: Int = 0): Seq[FuncProvider] = {
    t.dealias match {
      ...
      case t@Func1Extractor(_, _) if t.find(_ proper_<:< Types.varType).isDefined =>
        val varSymbols = t.collect.filter(_ proper_<:< Types.varType)
      ...
      case t if t proper_<:< Types.objectProviderTpe =>
  ...
  def argsFromA(a: Type): Args = {
    a match {
      case LabeledExtractor(l, a) =>
        if (a proper_<:< Types.coproductType) CoproductArgs(Types.split2ArgsRec(a, Types.cconsType).map(argsFromA), Some(l))
        else if (a proper_<:< Types.hlistType) ListArgs(Types.split2ArgsRec(a, Types.hconsType).map(argsFromA), Some(a))
        else SingleArg(a, Some(l))
      case a =>
        if (a proper_<:< Types.coproductType) CoproductArgs(Types.split2ArgsRec(a, Types.cconsType).map(argsFromA), None)
        else if (a proper_<:< Types.hlistType) ListArgs(Types.split2ArgsRec(a, Types.hconsType).map(argsFromA), Some(a))
  ...
  class ExprTreeBuilder(eval: u.Tree => Any) extends generation.exprs.ExprTreeBuilder[({type W[_] = Tree})#W](eval)
