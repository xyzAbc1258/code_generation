...
import generation.NoSubtyping
import generation.ObjectFuncProvider
import generation.HListUtils
import generation.MGenerator
import generation.NoLoops
import generation.Var1
import generation.Var2
import MazeSolver._
...
object MazeSolver {
  ...
  trait P[R, C] //Representation of free field in maze with coordinates (R, C) - row, column
  trait PT[R, C] //Representation of current posittion in maze

  //Custom representation of natural numbers to avoid issues with constraint that in shapeless.Succ[N] N must be subtype of Nat
  trait Z
  trait S[Z]
  ...
  //Maze representation as list of avaialable fields
  ...
  /**
   * Helper trait with methods representing moving in maze in four directions
   */
  trait MazeTraverser {
  ...
  def main(args: Array[String]): Unit = {
    val res3 = MGenerator.applyL[
      Nat._1, //Single solution
      M2._61, //Max depth 61
      ObjectFuncProvider[MazeTraverser]::HNil, //In context methods of MazeTraverser
      FieldType["traverser",MazeTraverser] :: Arr30, //As arguments list of aviailable fields and traverser
      PT[_0, _0] => PT[_29, _29], //Target: Function moving from field (0,0) to field (29,29)
      NoLoops with NoSubtyping //Generation options. No paths with loops, no subtyping for optimized type comparison
    ]
    res3.expressions
